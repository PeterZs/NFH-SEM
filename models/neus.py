...
class Poly4Network(nn.Module):
    def __init__(self, c_init=0.5, d_init=0.0, e_init=0.0):
        super(Poly4Network, self).__init__()

        mean_values = np.array([c_init, c_init, c_init, c_init, e_init, e_init, e_init, e_init]) 
        var_values = np.array([d_init, d_init, d_init, d_init])
        poly_init_values = np.array([0.0, 0.0, 0.0, 0.0])
        connected_array = np.concatenate((var_values, poly_init_values))
        self.register_parameter('bsd_c', nn.Parameter(torch.tensor(mean_values, dtype=torch.float32)))
        self.register_parameter('bsd_d', nn.Parameter(torch.tensor(connected_array, dtype=torch.float32)))

    def forward(self):
        return self.bsd_c, self.bsd_d
...
class NeuSModel(BaseModel):
    def setup(self):
        ...
        self.bsd_network = Poly4Network(c_init=self.config.bsd.c_init, d_init=self.config.bsd.d_init, e_init=self.config.bsd.e_init)
    ...
    def update_step(self, epoch, global_step):
        ...
        update_module_step(self.bsd_network, epoch, global_step)
